...
;read sectors command: 0x20, write 28 bit: 0x30, write 48 bit: 0x34

dd_prtlen equ 0x80
dd_dcr equ 0x0  ;device controll register
dd_stLBA equ 0x1
dd_tf equ 0x0
dd_sbits equ 0x0
...
;outb (0x1F2, sectorcount high)dd_sbits
;outb (0x1F3, LBA4)
...
;_______write_sectors_______________________________________________________
pio28_write:
	add ebp, [esi + dd_stLBA]	; convert relative LBA to absolute LBA
	mov ecx, ebp			; save a working copy
	mov al, bl		; set al= sector count (0 means 256 sectors)
	...
	inc edx			; port 1f7 -- command/status
	mov al, 0x30		; send "write" command to drive
	out dx, al
